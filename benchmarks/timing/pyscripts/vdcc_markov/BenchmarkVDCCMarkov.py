...
def vdcc_markov_ineff(n_channels, trange, dt):
    ...
    for t_index in range(0, len(trange) - 1):
        ...
        num = len(np.nonzero(states[:, t_index] == 0))
        n0[t_index] = num

        num = len(np.nonzero(states[:, t_index] == 1))
        n1[t_index] = num

        num = len(np.nonzero(states[:, t_index] == 2))
        n2[t_index] = num

        num = len(np.nonzero(states[:, t_index] == 3))
        n3[t_index] = num

        num = len(np.nonzero(states[:, t_index] == 4))
        n4[t_index] = num
