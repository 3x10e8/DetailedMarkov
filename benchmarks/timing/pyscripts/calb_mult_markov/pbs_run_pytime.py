...
if __name__ == '__main__':
  ...
  if (len(sys.argv)<3):
    ...
    script_exec = './BenchmarkCalbindinMarkovOpt.py' # path name to python script
    script_cmd = 'source ~/.bashrc ; %s $seed' % (script_exec)


    job_script_template = '%s; for seed in ${seeds[@]}; do { %s & }; done; wait;'

    job_name_template = 'rt_%04d_%04d'
  ...
  for start_i in range(0,len(seed_list),seeds_per_node):
    ...
    job_script = job_script_template % (seed_chunk, script_cmd)
